    ...
    async handleSlash(line, startup) {
        ...
        switch (cmd) {
            ...
            case 'usage': {
                ...
                const meter = this.dsh.measureTokens(agent.session);
                const cachePart = totals.cacheRate !== undefined ? ` · cache: ${totals.cacheRate}` : (totals.cacheHit && totals.cacheHit > 0 ? ` · cache: ${String(totals.cacheHit)}` : '');
                const speedPart = totals.tps !== undefined && totals.tps > 0 ? ` · speed: ${String(totals.tps)} tps` : '';
                this.out(`  responses: ${String(totals.responses)} · in: ${String(totals.input)} · out: ${String(totals.output)}${cachePart}${speedPart}${meter === undefined ? '' : ` · ctx: ${String(meter.totalTokens)} tok`}\n`);
                return 'continue';
            }
            case 'tools': {
                if (agent === undefined)
