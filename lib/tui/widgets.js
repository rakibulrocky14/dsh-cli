...
export function Banner({ model, effort, cwd, recent, width = MAX_CONTENT }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", width: boxWidth, children: [_jsxs(Box, { flexDirection: "row", width: boxWidth, alignItems: "flex-start", children: [_jsxs(Box, { flexDirection: "column", width: leftWidth, marginRight: 2, alignItems: "center", children: [WHALE.map((line, i) => (_jsx(Text, { color: theme.accent, children: line.padEnd(whaleWidth) }, i))), _jsx(Text, { bold: true, color: theme.accent, children: "deepseek" })] }), _jsxs(Box, { flexDirection: "column", width: rightWidth, justifyContent: "center", children: [_jsxs(Text, { bold: true, color: theme.accent, children: ["DeepSeek Terminal ", _jsx(Text, { dimColor: true, children: "\u00B7 Welcome back!" })] }), _jsx(Text, { dimColor: true, children: clipWidth(info, rightWidth) }), _jsx(Text, { dimColor: true, children: clipWidth(cwd, rightWidth) }), _jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Text, { color: theme.warn, children: "Recent activity" }), _jsx(Text, { dimColor: true, children: recent === undefined || recent === '' ? 'No recent activity' : clipWidth(recent, rightWidth) })] })] })] }), _jsxs(Box, { flexDirection: "column", marginTop: 1, marginLeft: 1, children: [_jsx(Text, { color: theme.warn, children: "Tips for getting started" }), BANNER_TIPS.map(t => _jsx(Text, { dimColor: true, children: clipWidth(`  · ${t}`, boxWidth - 2) }, t))] })] }));
}
...
function MarkdownTable({ headers, rows, width, dimmed }) {
    ...
    const cells = (row) => Array.from({ length: cols }, (_, i) => row[i] ?? '');
    ...
    return (_jsxs(Box, { flexDirection: "column", marginTop: 1, marginBottom: 1, children: [_jsx(Text, { bold: true, color: dimmed ? undefined : theme.accent, dimColor: dimmed, children: line(cells(headers)) }), _jsx(Text, { dimColor: true, children: widths.map(w => '─'.repeat(w)).join('─┼─') }), rows.map((row, i) => (_jsx(Text, { dimColor: dimmed, children: line(cells(row)) }, i)))] }));
}
/**
...
export function RichText({ text, dimmed = false, width = MAX_CONTENT }) {
    ...
    return (_jsx(Box, { flexDirection: "column", children: sections.map((section, i) => {
            if (section.code) {
                return (_jsxs(Box, { flexDirection: "column", marginY: 1, children: [section.lang === '' ? undefined : _jsxs(Text, { dimColor: true, children: ["  ", section.lang] }), _jsx(Text, { dimColor: dimmed, color: theme.code, children: section.text.split('\n').map(line => `  ${line}`).join('\n') })] }, i));
            }
            return (_jsx(Box, { flexDirection: "column", children: parseMarkdown(section.text).map((block, j) => {
                    switch (block.kind) {
                        case 'heading':
                            return (_jsx(Box, { marginTop: j === 0 ? 0 : 1, children: _jsx(Text, { bold: true, color: dimmed ? undefined : theme.accent, dimColor: dimmed, children: block.text }) }, j));
                        case 'hr':
                            return _jsx(Text, { dimColor: true, children: '─'.repeat(Math.min(24, Math.max(8, width - 4))) }, j);
                        case 'quote':
                            return (_jsx(Box, { flexDirection: "column", marginLeft: 1, children: block.lines.map((line, k) => (_jsxs(Text, { dimColor: true, italic: true, children: ["\u2502 ", line] }, k))) }, j));
                        case 'list':
                            return (_jsx(Box, { flexDirection: "column", children: block.items.map((item, k) => (_jsxs(Text, { dimColor: dimmed, children: [block.ordered ? `${String(k + 1)}. ` : '· ', _jsx(InlineText, { text: item, dimmed: dimmed })] }, k))) }, j));
                        case 'table':
                            return _jsx(MarkdownTable, { headers: block.headers, rows: block.rows, width: width, dimmed: dimmed }, j);
                        case 'para':
                            return (_jsx(Box, { marginTop: j === 0 ? 0 : 1, children: _jsx(InlineText, { text: block.text, dimmed: dimmed }) }, j));
                    }
                }) }, i));
        }) }));
}
...
export function BlockView({ block, width = MAX_CONTENT, expanded = false, spinnerFrame = 0 }) {
    switch (block.kind) {
        ...
        case 'assistant': {
            ...
            return (_jsxs(Box, { flexDirection: "column", children: [block.live ? _jsx(Text, { color: theme.success, children: "\u273B assistant" }) : undefined, body.hidden > 0 ? _jsxs(Text, { dimColor: true, children: ["\u2026 +", String(body.hidden), " lines above"] }) : undefined, _jsx(RichText, { text: body.text, width: width })] }));
        }
        case 'reasoning': {
 ...
 */
export function terminalHeight(stdoutRows) {
 ...
 */
export function Composer({ field, focused = true, running = false, spinnerFrame = 0, runSeconds = 0, tps, width = MAX_CONTENT, placeholder, model, effort, }) {
    ...
    return (_jsx(Box, { flexDirection: "column", width: width, children: _jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: running ? theme.warn : theme.accent, paddingX: 1, width: width, children: [_jsx(Box, { flexDirection: "row", width: innerWidth, children: _jsx(FieldView, { label: label, field: field, focused: focused, placeholder: placeholder ?? (running ? runHint : idleHint) }) }), _jsxs(Box, { flexDirection: "row", justifyContent: "space-between", width: innerWidth, marginTop: 1, children: [_jsx(Box, { children: running ? (_jsx(Text, { color: theme.warn, children: leftTag })) : showLeft && leftTag !== '' ? (_jsxs(Text, { color: theme.accent, children: ["\u25C8 ", clipWidth(leftTag, innerWidth - (showRight ? displayLen(rightTag) + 2 : 0))] })) : undefined }), _jsx(Box, { children: showRight ? _jsx(Text, { dimColor: true, children: rightTag }) : undefined })] })] }) }));
}
...
export function Panel({ title, rows, loading, hint, index, maxRows = 20, spinnerFrame = 0, width = MAX_CONTENT }) {
    ...
    const usable = Math.max(20, width - 4);
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.border, paddingX: 1, width: width, children: [_jsxs(Text, { bold: true, color: theme.accent, children: ["\u25B8 ", clipWidth(title, usable), rows.length > 0 && !loading ? _jsxs(Text, { dimColor: true, children: [" (", String(rows.length), ")"] }) : undefined] }), loading ? _jsxs(Text, { dimColor: true, children: [spinnerGlyph(spinnerFrame), " loading\u2026"] }) : undefined, !loading && rows.length === 0 ? _jsx(Text, { dimColor: true, children: "(empty)" }) : undefined, start > 0 ? _jsxs(Text, { dimColor: true, children: ["\u2026 ", String(start), " more above"] }) : undefined, visible.map((row, i) => {
...
export function Footer({ model, effort, cwd, ctxTokens, ctxWindow, mode, cacheRate, tps, running, modalOpen, inPanel, width }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { children: [_jsx(Text, { bold: true, color: theme.accent, children: fit.model }), _jsx(Text, { dimColor: true, children: " \u00B7 " }), _jsx(Text, { color: theme.accent, children: fit.effort }), _jsx(Text, { dimColor: true, children: " \u00B7 " }), _jsx(Text, { dimColor: true, children: fit.cwd }), fit.ctx === undefined ? undefined : (_jsxs(_Fragment, { children: [_jsx(Text, { dimColor: true, children: " \u00B7 " }), _jsx(Text, { dimColor: true, children: fit.ctx })] })), fit.cache === undefined ? undefined : (_jsxs(_Fragment, { children: [_jsx(Text, { dimColor: true, children: " \u00B7 " }), _jsx(Text, { color: theme.success, children: fit.cache })] })), fit.tps === undefined ? undefined : (_jsxs(_Fragment, { children: [_jsx(Text, { dimColor: true, children: " \u00B7 " }), _jsx(Text, { color: theme.accent, children: fit.tps })] })), _jsx(Text, { dimColor: true, children: " \u00B7 " }), mode === ''
