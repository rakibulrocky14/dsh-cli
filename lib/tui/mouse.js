function parseDecimal(value, maxDigits) {
 ...
 */
export function parseMouseReport(input) {
    if (!input.startsWith('[<'))
        return undefined;
    ...
    if (button === undefined || column === undefined || row === undefined || column < 1 || row < 1)
        return undefined;
    return {
        button,
        x: column - 1,
...
export function wheelDirection(report) {
