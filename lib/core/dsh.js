...
export class Dsh {
    ...
    readTokenUsage(session) {
        const projections = service(this.ctx, 'sessionProjections');
        if (projections === undefined)
            return undefined;
        try {
            if (hasMethod(projections, 'stateOf')) {
                const state = projections.stateOf.call(projections, session, 'tokenUsage');
            ...
            if (hasMethod(projections, 'snapshot')) {
                const snap = projections.snapshot.call(projections, session);
    ...
    readSessionStats(session) {
        ...
        try {
            let stats;
            if (hasMethod(projections, 'stateOf')) {
                stats = projections.stateOf.call(projections, session, 'sessionStats');
            }
