    ...
    for (const event of events) {
        if (event.type === 'turn/start') {
            ...
            if (openStep !== undefined && openStep.firstTokenTime === undefined) {
                const turn = typeof event.data?.turn === 'number' ? event.data.turn : openStep.turn;
                const step = typeof event.data?.step === 'number' ? event.data.step : openStep.step;
    ...
    liveTps(fallbackStart) {
        ...
        if (startTime !== undefined) {
            ...
            if (elapsed >= 0.3) {
                ...
                if (outTokens > 0)
                    return Math.max(1, Math.round(outTokens / elapsed));
            }
        }
