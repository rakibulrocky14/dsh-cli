...
export function formatCacheHitRate(hit: number, totalInput: number, miss?: number): string | undefined {
  if (hit <= 0) return undefined
  const total = miss !== undefined && miss > 0 ? (hit + miss) : totalInput
  if (total <= 0) return undefined
  const rate = Math.min(100, Math.max(0, (hit / total) * 100))
