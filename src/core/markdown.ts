...
function isRule(line: string): boolean {
  ...
  for (const c of line) if (c !== ch) return false
  return true
}
...
function orderedPrefix(line: string): number {
  let i = 0
  ...
  if (line[i + 2] === undefined) return 0
  return i + 2
}
...
function isBlockStart(lines: string[], idx: number): boolean {
  ...
  if (line === '') return true
  if (headingLevel(line) > 0) return true
  if (isRule(line)) return true
  if (isTableRow(line) && idx + 1 < lines.length && isTableSep(lines[idx + 1]!)) return true
  if (isBullet(line) || orderedPrefix(line) > 0) return true
