...
export const EFFORT_LEVELS: readonly string[] = ['off', 'low', 'high', 'max']
...
export function normalizeEffort(
  text: string,
  allowed: readonly string[] = EFFORT_LEVELS,
): { clear: true } | { level: string } | undefined {
  const trimmed = text.trim().toLowerCase()
  if (trimmed === '') return undefined
  if (trimmed === 'auto' || trimmed === 'default') return { clear: true }
  const hit = allowed.find(level => level.toLowerCase() === trimmed)
  if (hit !== undefined) return { level: hit }
  throw new Error(`unknown effort "${text.trim()}" — use ${allowed.join('|')} or auto`)
}

/** Parse `/model` selection text into provider/model parts. */
export function parseModelSelection(text: string): { provider: string; model: string } | undefined {
...
export function parseAssignments(args: string[]): Record<string, unknown> {
  ...
  for (const arg of args) {
    ...
    if (eq <= 0) throw new Error(`expected k=v, got "${arg}"`)
...
export function shortSession(id: string): string {
  ...
  if (!id.startsWith('session-')) return id.length <= 16 ? id : id.slice(0, 16)
