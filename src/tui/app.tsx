...
const HistoryRegion = memo(function HistoryRegion({ entries, repaintSeq, width, expanded }: {
  ...
  return (
    <Static key={repaintSeq} items={entries}>
      {(entry) => entry.item.kind === 'gap'
        ? <Text key={entry.item.id}>{'\n'.repeat(24)}</Text>
        : entry.item.kind === 'banner'
          ? <Banner key={entry.item.id} model={entry.item.model} effort={entry.item.effort} cwd={entry.item.cwd} recent={entry.item.recent} width={width} />
          : <BlockView key={entry.item.id} block={entry.item} width={width} expanded={expanded} />}
    </Static>
  )
})
 ...
 */
export function App({ engine, startup }: { engine: Engine; startup: StartupValues }): React.JSX.Element {
  ...
  return (
    <Box flexDirection="column" width={frameWidth}>
      <Box flexDirection="column" paddingX={1}>
        <HistoryRegion entries={staticEntries} repaintSeq={engine.repaintSeq} width={contentWidth} expanded={engine.toolsExpanded} />
...
export function startTui(ctx: DshContext, startup: StartupValues, exit: (code: number) => void): void {
  ...
  const app = render(<App engine={engine} startup={startup} />)
