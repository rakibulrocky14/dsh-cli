 ...
 */
import { Dsh, attachLiveStream, sendFollowup } from './core/dsh.js';
 ...
 */
export async function runPrint(ctx, startup, io) {
    ...
    feed.subscribe(() => {
        ...
        if (live.text.length <= printedReasoning)
            return;
        if (!reasoningOpen) {
    ...
    const detach = attachLiveStream(ctx, agent, feed, (event) => {
        if (event.type !== 'assistant/chunk')
            feed.notifyCommitted(readSessionEvents(agent.session));
    });
